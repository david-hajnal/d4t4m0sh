# mosh_algorithms/video_to_image_mosh.py
import os, json, shutil, subprocess, tempfile
import av  # used to find I-frames deterministically
...
def _collect_iframes(path):
    """Return list of absolute frame indices where pict_type is I (or key)."""
    idxs = []
...
def _make_image_motion_clip(img_path, out_path, w, h, fps_str, dur, mpeg4_gop, kb_mode, verbose=False):
    """
    Create a short motion clip from a still image to seed motion vectors.
    kb_mode:
      - 'rotate' (robust): tiny sinusoidal rotation jitter (default)
      - 'zoom_in' (uses zoompan)
    """
    if kb_mode == "zoom_in":
        # gentle zoom from 1.0 -> 1.15 across duration using zoompan
        zoom_step = 0.0015
        vf = (
            f"scale={w}:{h},"
        ...
        # rotate jitter (very stable across ffmpeg builds)
        vf = (
            f"scale={w}:{h},"
...
def process(input_path: str, output_path: str, alpha=0.85, block=16, radius=8,
    ...
    """
    Video -> Image datamosh preset:
      - Normalize the video (CFR, even WxH) to a mosh-friendly AVI.
      - Build a motion clip from the still image (rotate jitter or zoom-in), same WxH/FPS.
      - Concat [video, image-clip] into one AVI.
      - Find all I-frames in the combined stream; drop ALL except frame 0,
        and also drop N 'postcut' frames after each boundary I.
      - Final encode (single pass) with huge GOP / no B-frames (AVI mpeg4 or MP4 libx264).

    Params:
      image: path to the still image (required)
      img_dur: seconds for the image clip
      kb_mode: 'rotate' (default) or 'zoom_in'
      postcut: frames to drop after each dropped I (recommended 3-6)
    """
    ...
    # probe video for normalization baseline
    ...
    with tempfile.TemporaryDirectory(prefix="vid2img_mosh_") as tmp:
        # 1) Normalize video to AVI/MPEG-4 (even WxH + CFR)
        ...
        # 2) Build image motion clip (AVI/MPEG-4) with same baseline
        ...
        # 3) Concat [video, image_clip]
        combined = os.path.join(tmp, "combined.avi")
        _run([
            "ffmpeg","-y","-i", norm_vid,"-i", img_clip,
        ...
        # 4) Compute which frames to drop: all I except very first (frame 0), plus 'postcut' after each
        ...
        # 5) Final single encode after select
        if wants_mp4:
